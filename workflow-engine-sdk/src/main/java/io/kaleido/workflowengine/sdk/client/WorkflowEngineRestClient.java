...
/**
 * Thin HTTP client for workflow engine REST operations (workflow, transaction,
 * and stream CRUD).
 *
 * <p>This is separate from {@link WorkflowEngineClient}, which is the primary
 * entry point for hosting handlers over WebSocket. Prefer that client for
 * provider applications.
 *
 * <p>TODO: Not currently used by the Java SDK surface or tests. Ported from the
 * TypeScript {@code WorkflowEngineRestClient}; needs more testing and samples
 * if it's going to live on.
 */
    ...
    public WorkflowEngineRestClient(ClientConfig config) {
        ...
        if (config.url() != null) {
            ...
            if (account == null || account.isEmpty())
                throw SDKErrors.newError(SDKErrors.MSG_ACCOUNT_NOT_SET);
            if (environment == null || environment.isEmpty())
                throw SDKErrors.newError(SDKErrors.MSG_ENVIRONMENT_NOT_SET);
            if (workflowEngine == null || workflowEngine.isEmpty())
                throw SDKErrors.newError(SDKErrors.MSG_WORKFLOW_ENGINE_NOT_SET);
            ...
            return httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                    .thenApply(response -> {
                        ...
                        if (response.statusCode() >= 400) {
                            throw SDKErrors.newError(SDKErrors.MSG_REST_REQUEST_FAILED,
                                    method + " " + url + " returned " + response.statusCode() + ": " + response.body());
                        }
                        try {
                            return JSON.MAPPER.readTree(response.body());
                        } catch (Exception e) {
                            throw SDKErrors.newError(e, SDKErrors.MSG_REST_REQUEST_FAILED,
                                    "Failed to parse response from " + url);
                        }
                    });
        } catch (Exception e) {
            return CompletableFuture.failedFuture(
                    SDKErrors.newError(e, SDKErrors.MSG_REST_REQUEST_FAILED, "Request failed: " + url));
        }
    }
}
